...
- **日志类型修复（v2.24，关键）**：数据工具页新增「修复日志类型」——把**被故障记录指向、却显示为「通过」**的日志自动重分类为「故障」日志。成因：v2.18 之前产生的数据（及后来被旧格式快照/旧备份合并进来的行）没有 `logType` 字段，系统迁移默认值=0，于是"故障/消除"看着像消失了，其实日志行与故障记录（现象/解决/已解决状态）一直都在，只是归类标注被冲成「通过」。修复只改类型标注、不增删数据，刷新时间戳后下次同步以"新者胜"把正确类型传播回全队；消除信息保留在故障记录"已解决"状态，不虚构消除日志
...
- **数据工具**：
  ...
  - **修复日志类型**（v2.24）：把被故障记录指向却显示为「通过」的日志重分类为「故障」，恢复因旧版本/旧快照合并被冲掉的类型标注
- **测试账号与 WebDAV 同步**：
  ...
  - 合并规则：按 UUID 主键去重、同 ID 冲突保留 `updatedAt` 较新者；**删除通过墓碑传播且同样"新者胜"**（v2.9 起随快照携带 `deletedItems`）：合并端先落库远端墓碑，再把每行墓碑的 `deletedAt` 与本机存活行 ∪ 快照携带行的最新 `updatedAt` 比对——删除早于该行更新 → 墓碑"被击败"、自清理并保留/插入该行；删除晚于该行已知全部更新 → 才应用删除（含级联）。既防止被删数据借旧快照复活，也防止旧机/旧快照里的旧删除清掉其他手机上较新的故障/消除/通过数据（v2.21）
